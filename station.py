...
def angle_for_temp(t):
  ...
  return TEMP_ANGLE_END - int(math.floor((t - angle_low) * (angle_range / temp_range) + TEMP_ANGLE_START))
...
def show_rain(d):
  ...
  for minute in d["minutes"]:
    if minute["precipProbability"] >= .5:
      first_rain = datetime.utcfromtimestamp(minute["time"]).replace(tzinfo=tzutc()).astimezone(EASTERN)
      break
...
def show_tides(d):
  ...
  first_tide = datetime.strptime(d["lowTides"][0], '%Y-%m-%d %H:%M').replace(tzinfo=EASTERN)
  minutes = first_tide.hour * 60 + first_tide.minute
  minutes = min(minutes, TIDE_MINUTES_END)

  angle = int(
    (minutes - TIDE_MINUTES_START) *
    ((TIDE_ANGLE_END - TIDE_ANGLE_START) /
    (TIDE_MINUTES_END - TIDE_MINUTES_START) + TIDE_ANGLE_START))
